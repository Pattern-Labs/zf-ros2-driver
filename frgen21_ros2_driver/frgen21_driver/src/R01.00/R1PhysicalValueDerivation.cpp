...
using namespace ZF_FRGen21_PTL_Interface_R01_00;

// ***************** static variables ******************

const float R1PhysicalValueDerivation::fRCS_Min =
    -1000.0f; /**< Minimum value to avoid log(0) */
const float R1PhysicalValueDerivation::fSpeedOfLight =
    299792458.0f; /**< Speed of electromagnetic waves [m/s] */
const float R1PhysicalValueDerivation::fRCS_TemperatureCalibrationFactor =
    0.0944f; /**< Compensate temperature dependence of signal power [dB/Celsius]
              */
const float R1PhysicalValueDerivation::fRCS_TemperatureCalibrationOffset =
...
const float R1PhysicalValueDerivation::fAntennaSpacingHorizontal =
    0.00906106f; /**< Antenna spacing of URA [m] */
const float R1PhysicalValueDerivation::fAntennaSpacingVertical =
...
template <typename T>
static T ZF_FRGen21_PTL_Interface_R01_00::clip(const T &value,
                                               const T &lowerLimit,
                                               const T &upperLimit) {
  return std::max(lowerLimit, std::min(value, upperLimit));
}

static int ZF_FRGen21_PTL_Interface_R01_00::round(const float &fValue) {
  return (fValue < 0.0f) ? (int)(fValue - 0.5f) : (int)(fValue + 0.5f);
}

template <typename T>
static T ZF_FRGen21_PTL_Interface_R01_00::square(const T &fValue) {
  return fValue * fValue;
}

// ***************** Public functions ******************

R1PhysicalValueDerivation::R1PhysicalValueDerivation() { m_bInit = false; }

R1PhysicalValueDerivation::~R1PhysicalValueDerivation() {}

int R1PhysicalValueDerivation::CalculateDerivedTargetValues(
...
void R1PhysicalValueDerivation::GetSoftwareVersion(
    const Struct_Itf4Eth_Primary_Data_t *ptr_PointTargetList_PrimaryData,
 ...
 **/
float R1PhysicalValueDerivation::RangeMapping(float fRangeFrequency,
 ...
 **/
float R1PhysicalValueDerivation::VelocityMapping(float fRange,
                                                 float fDopplerFrequency,
...
float R1PhysicalValueDerivation::AzimuthMapping(
...
float R1PhysicalValueDerivation::ElevationMapping(
...
float R1PhysicalValueDerivation::AntennaGain(float fAzimuth, float fElevation) {
...
float R1PhysicalValueDerivation::FrequencyFilterGain(float fRange) {
  return LinearInterpolation(fRange / m_sFrequenyFilterGainLUT.fStepSize,
                             &m_sFrequenyFilterGainLUT.fLUT[0],
                             SizeOfIF_FilterLUT);
}

float R1PhysicalValueDerivation::LinearInterpolation(float fx, float *fValues,
                                                     size_t numValues) {
...
float R1PhysicalValueDerivation::CalcRCS(float fPower, float fRCS_Calibration,
...
void R1PhysicalValueDerivation::FrequencyShiftAAR(
