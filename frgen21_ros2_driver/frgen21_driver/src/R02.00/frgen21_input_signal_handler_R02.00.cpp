...
namespace R02_00 {
InputSignalHandlerR02_00::InputSignalHandlerR02_00(const std::string &remote_ip,
                                                   int remote_port,
                                                   int local_port)
...
void InputSignalHandlerR02_00::set_mode(uint8_t range_mode) {
  // create object from struct
  CommsInputSignalsR02_00 input_signals;
  ...
  // calculate CRC
  uint16_t crc =
      CalculateCRC16_P05(reinterpret_cast<uint8_t *>(&input_signals), 2,
                         sizeof(input_signals) - sizeof(input_signals.CMAC) -
                             sizeof(input_signals.AntiReplayCounter),
                         0xFFFF);
  uint16_t crc_final =
  ...
  std::vector<char> output_data =
      std::vector<char>(ptr, ptr + sizeof(CommsInputSignalsR02_00));
