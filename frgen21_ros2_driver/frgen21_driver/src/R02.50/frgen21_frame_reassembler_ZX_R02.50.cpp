...
namespace R02_50 {
FrameReassemblerR02_50::FrameReassemblerR02_50(
...
void FrameReassemblerR02_50::prepareNewFrame() {
  scan_id_.reset();
  packet_id_.reset();
  frame_size_ = 0;
...
template <typename T>
void FrameReassemblerR02_50::processPackage(const std::vector<char> &data,
                                            T *buffer_ptr) {
...
void FrameReassemblerR02_50::processPackage(const std::vector<char> &data,
  ...
  if (position <= ZF_FRGen21_PTL_Interface_ZX_R02_50::NumberOfTargetPackages) {
...
FrameStatus FrameReassemblerR02_50::pushData(const std::vector<char> &data) {
  ...
  switch (data_length) {
  // header: 672
  case sizeof(ZF_FRGen21_PTL_Interface_ZX_R02_50::Struct_ZX_Itf4Eth_Header_t):
  ...
  case sizeof(ZF_FRGen21_PTL_Interface_ZX_R02_50::
  ...
  case sizeof(ZF_FRGen21_PTL_Interface_ZX_R02_50::
    ...
    processPackage(data, &radar_data_container_buffer_->sBlockageDynCoeffs);
    break;

  // blockage_coeff_OPE + AntiReplayCounter + CRC: 810
  case sizeof(ZF_FRGen21_PTL_Interface_ZX_R02_50::
                  Struct_ZX_Itf4Eth_Payload_Blockage_Dyn_Coeff_OPE_t) +
      PTL_CS_BYTES_SIZE:
    processPackage(data, &radar_data_container_buffer_->sBlockageDynCoeffsOPE);
    break;
...
std::shared_ptr<radar_frame_t> FrameReassemblerR02_50::getFrameAsStruct() {
...
std::shared_ptr<char[]> FrameReassemblerR02_50::getFrame() {
  if (frame_status_ == FrameStatus::FRAME_COMPLETE) {
    char *tmp_package =
...
std::size_t FrameReassemblerR02_50::getSize() { return sizeof(radar_frame_t); }
